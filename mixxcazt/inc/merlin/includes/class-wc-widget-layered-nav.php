...
class Mixxcazt_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce mixxcazt_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Mixxcazt custom)', 'mixxcazt');
        $this->widget_id          = 'mixxcazt_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Mixxcazt Filter Products by Attribute', 'mixxcazt');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'mixxcazt'),
                'label' => esc_html__('Title', 'mixxcazt'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'mixxcazt'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'mixxcazt'),
                'options' => array(
                    'and' => esc_html__('AND', 'mixxcazt'),
                    'or'  => esc_html__('OR', 'mixxcazt'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="mixxcazt-button-type" href="' . esc_url($link) . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="mixxcazt-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="mixxcazt-image-type" href="' . esc_url($link) . '">';
